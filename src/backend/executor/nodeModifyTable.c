					 ...
					 TupleTableSlot **rslot)
{
	...
	{
		/* Initialize "existing" - in the else branch, 
		   this is done by ExecOnConflictLockRow */
		bool found = table_tuple_fetch_row_version(relation, conflictTid, 
			                                         SnapshotAny, existing);
		/* Even if the tuple is deleted, it must still be physically present */
		Assert(found);
	}
	else
	{
