 ...
 * oldSlot: slot holding old tuple deleted or updated, or existing tuple
					 ...
					 TupleTableSlot **rslot)
{
	...
	if (resultRelInfo->ri_WithCheckOptions != NIL)
	{
		 ...
		 * security barrier quals (if any), enforced here as RLS checks/WCOs.
		 *
		 * The rewriter creates WCOs from the USING clauses of SELECT
		 * policies, and stores them as WCOs of "kind" WCO_RLS_CONFLICT_CHECK.
		 * If FOR UPDATE/SHARE was specified, UPDATE rights are required on
		 * the target table, so the rewriter also adds WCOs from the USING
		 * clauses of UPDATE policies (not the WITH CHECK clauses), using WCOs
		 * of the same kind, so this check enforces them too.
		 */
		ExecWithCheckOptions(WCO_RLS_CONFLICT_CHECK, resultRelInfo,
							 existing,
